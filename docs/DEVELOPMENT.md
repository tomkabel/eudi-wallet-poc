...
not mean here. The checks that define green live in [`../AGENTS.md`](../AGENTS.md) under
"Running the checks locally"; this file covers the prerequisites they assume and the
runnable flows beyond them.
...
- **Tools**: `jq` and `ip` (iproute2) for `demo/demo.sh`; `curl` for `tests/e2e.sh`;
  `shellcheck` for the checks in AGENTS.md.
- **Submodules**: `git submodule update --init` — both URLs are SSH, so it needs GitHub SSH
  access; needed for the spec baseline and the Android UI, not for the two flows below.

## The Longfellow sibling checkout

`verifier/zkverify-ffi` path-depends on `../../../longfellow-zk`, so `google/longfellow-zk`
must sit **next to** this repository (`<parent>/ee-eudiw`, `<parent>/longfellow-zk`). The
...
per-process concurrency limit); the wallet README notes there is no UI and no WSCD. The
Android demo records the UI beside the terminal — the UI does not talk to the verifier, and
the proof on screen is generated by `ee_poa_demo` on the laptop.
